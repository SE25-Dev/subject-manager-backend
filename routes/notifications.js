 ...
 */
notifications_router.get("/notifications", verifyTokenAndExtractUser, async (req, res) => {
 ...
 */
notifications_router.put("/notifications/:notificationId/mark_as_read", verifyTokenAndExtractUser, async (req, res) => {
    ...
    try {
        ...
        if (!notification) {
            return res.status(404).json({ error: "Notification not found or you don't have permission to update it." });
        }
        ...
        if (notification.type === 'raport_section_addition') {
            ...
            if (sectionId) {
                ...
                if (allRead) {
                    ...
                    if (activeStatus) {
                        await Section.update({ statusId: activeStatus.id }, { where: { id: sectionId } });
                    }

                    // Delete all notifications for this section
                    await Notification.destroy({ where: { sectionId: sectionId } });
                    return res.status(200).json({ message: "All section notifications marked as read, section activated, and notifications deleted.", sectionId: sectionId });
                }
            }
        } else {
            await notification.destroy();
            return res.status(200).json({ message: "Notification marked as read and deleted.", notificationId: notificationId });
        }

        res.status(200).json({ message: "Notification marked as read.", notification: notification });
