...
const { verifyTokenAndExtractUser, verifySuperuser } = require('../../helpers/authMiddleware');
 ...
 */
courses_router.post("/:courseId/enroll", verifyTokenAndExtractUser, async (req, res) => {
 ...
 */
courses_router.post("/course_creation_request", verifyTokenAndExtractUser, async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: "Course creation request submitted successfully.", request: newRequest });

    } catch (error) {
        console.error("Error submitting course creation request:", error);
        res.status(500).json({ error: "Failed to submit course creation request." });
    }
});

courses_router.put("/course_creation_request/:requestId", verifyTokenAndExtractUser, verifySuperuser, async (req, res) => {
    ...
    try {
        ...
        if (!request) {
            return res.status(404).json({ error: "Course creation request not found." });
        }
        ...
        if (action === "accept") {
            ...
            if (!headTeacherRole) {
                return res.status(500).json({ error: "HeadTeacher role not found. Please ensure roles are seeded." });
            }
            ...
            res.status(200).json({ message: "Course creation request accepted and course created.", course: newCourse });
        ...
        res.status(500).json({ error: "Failed to handle course creation request." });
    }
});
 ...
 */
courses_router.get("/course_creation_requests", verifyTokenAndExtractUser, verifySuperuser, async (req, res) => {
    try {
        ...
        res.status(500).json({ error: "Failed to fetch course creation requests." });
    }
});
